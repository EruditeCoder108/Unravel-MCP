...
*Source-verified against the current `unravel-mcp` implementation. MCP path and web-app path documented separately. Last updated 2026-05-01: MCP tools modularized under `server/tools/`; `consult` is intentionally paused; MCP `doctor` added; benchmark runner now has clean temp outputs, routing/full modes, all ground-truth package routing coverage, and AST structural facts for constructor arguments plus property accesses.*

> [!IMPORTANT]
> **Current MCP status (2026-05-01):** The active MCP tools are `analyze`, `verify`, `build_map`, `query_graph`, `query_visual`, `doctor`, and paused `consult`. `consult` currently returns `TEMPORARILY_PAUSED`; use `build_map`, `query_graph`, `analyze`, `verify`, and `doctor` for the reliability workflow.
...
### 1a. What Unravel Is
> "Unravel is a deterministic bug-diagnosis engine. It uses static AST analysis to extract verified structural facts from code: mutation chains, closure captures, async boundaries, race conditions, constructor call arguments, and property access maps. Detector findings are treated as strong evidence; constructor/property maps are leads that still need symptom/spec verification."
...
User reports bug →
  ...
  Need health/status? → doctor(directory) → inspect KG freshness, call edges, memory, embeddings
...
- Constructor call sites (structural leads): `new ClassName(arg0=...)` with line, enclosing function, and flags for `null`, `undefined`, and type assertions
- Property access maps (structural leads): object/property reads such as `event.key` and `event.code`, grouped by file/object so the agent can reason about semantic-property bugs

> [!IMPORTANT]
> Constructor call sites and property access maps are not bug verdicts. They are deterministic facts that help with bugs the older mutation detector missed: null-injection / wrong constructor argument flow (example: b-15) and semantic property choices (example: b-20 `event.key` vs `event.code`). The agent must still connect them to the symptom and cite source lines in `verify()`.
> This is now covered by an MCP contract test: `analyze().critical_signal` must render both sections, not merely store them in `raw_ast_data`.
...
- Cross-file call graph edges (MCP path) from native tree-sitter plus KG call edges from `ast-bridge.js` structural extraction. The former powers analyze-time cross-file evidence; the latter powers `build_map`/`query_graph`.

> [!NOTE]
> **Three structural paths, three scopes:** (1) `ast-engine-ts.js` (native tree-sitter, MCP analyze path) produces deep detector facts, constructor argument facts, property access maps, and analyze-time cross-file evidence. (2) `ast-bridge-browser.js` (WASM, browser path) produces browser KG call edges. (3) `ast-bridge.js` (Node KG path) now extracts imports, classes, functions, and best-effort call edges for `build_map`/`query_graph`.
...
**§C — Pattern Hints (only if patterns matched with confidence ≥ 0.65):**
```
...
> [!NOTE]
> **Hint threshold is 0.65 in the MCP response.** This means weak pattern matches are not promoted into `_instructions.patternHints`. The pattern store may still calculate lower-confidence matches internally for learning/penalty behavior, but the agent-facing H1 hint is gated more strictly.
...
> **Project health/status check.** `doctor` tells the agent what Unravel currently knows about a project before or during debugging. It does not diagnose a bug; it reports whether the Knowledge Graph and memory layers are present, fresh, and useful.

| Param | Type | Required | Description |
|---|---|---|---|
| `directory` | string | No | Project root. If omitted, uses `session.projectRoot` from the last `build_map`, `analyze`, or `query_graph` call |
| `checkFreshness` | boolean | No | Default `true`. Compares source file hashes against the saved KG |

**What it reports:**
- KG presence, indexed files, nodes, edges, call edges, import edges
- Embedded node count, schema version, engine version, build timestamp
- Pattern Store, Diagnosis Archive, Task Codex counts
- Codex stale/unreliable counts
- Freshness state and changed files
- Embedding provider readiness
- Suggestions such as "run build_map" or "call edges are zero"
...
```json
{
  "status": "ok",
  "projectRoot": "/repo",
  "kg": { "present": true, "nodes": 55, "edges": 80, "callEdges": 23 },
  "memory": { "patternStoreEntries": 40, "diagnosisArchiveEntries": 1, "taskCodexEntries": 12 },
  "freshness": { "checked": true, "stale": false, "reason": "graph is fresh" },
  "embeddings": { "provider": "gemini", "status": "active" },
  "suggestions": []
}
```
...
> **Temporarily paused.** `consult` is the planned Project Oracle, but in the current MCP it intentionally returns `TEMPORARILY_PAUSED` while output quality, evidence separation, and reliability gates are hardened. It is not the recommended path today.
...
**Current response:** The active handler returns a JSON status packet explaining that `consult` is paused and points users to `build_map`, `query_graph`, `analyze`, and `verify`.

**Historical/planned behavior:** The sections below describe the intended Oracle design that will return after the reliability gates are satisfied.

**Planned behavior:** Auto-builds KG on first call if none exists (~15-30s, one-time). Every subsequent call should be instant or run an incremental staleness check.
...
| `include` | string[] | No | Only index paths/folders matching these substrings. Useful for monorepos |
| `exclude` | string[] | No | Paths to skip — relative to project root or substrings. E.g. `["src/generated", "vendor/legacy"]` |
| `force` | boolean | No | Force a full rebuild even if a cached KG exists |
...
**Shared patch path (2026-05-01):** `build_map` incremental rebuilds and `query_graph` self-healing both use the same `patchKnowledgeGraph()` helper. This prevents two subtly different versions of import/call-edge repair from drifting apart.
...
2. Runs a freshness check against stored file hashes
   - Small stale KG: patches changed files in-place with `patchKnowledgeGraph()`, recomputes import/call edges, saves the healed graph, then routes against fresh data
   - Large stale KG: returns `graphFreshness.needsFullRebuild = true`
   - Changed embedded nodes are marked stale instead of silently trusted
3. If `GEMINI_API_KEY` set AND nodes have embeddings: embeds symptom as `RETRIEVAL_QUERY`, computes cosine against all embedded nodes → `semanticScores` Map
4. **Pattern-aware KG routing (§2.1, 2026-03-30):** `matchPatterns(session.astRaw)` → `getNodeBoosts()` boosts files associated with matched structural patterns. Merged into `semanticScores` via `Math.max`. MCP and webapp now have parity on this.
5. `queryGraphForFiles(graph, symptom, maxResults, semanticScores)`:
...
6. Searches `.unravel/codex/` via `searchCodex()`:
...
7. Assembles response, including `graphFreshness` and `embeddingProvider`
...
```json
{
  ...
  "fileCount": 12,
  "graphFreshness": { "checked": true, "stale": false, "reason": "graph is fresh" },
  "embeddingProvider": { "provider": "gemini", "status": "active" },
...
| Pure regex | `ast-bridge.js` | KG structural fallback / fast bridge | ✅ Best-effort calls via regex, including class methods and member calls as of 2026-05-01 |
...
The regex fallback also defines `AMBIGUOUS_STEMS` — common names (`utils`, `config`, `helpers`, etc.) where import resolution is skipped to avoid false cross-file links. It is less precise than tree-sitter, but it no longer emits empty `calls: []` by design.
...
- That `ast-bridge.js` call extraction is best-effort regex — useful for KG routing, but tree-sitter remains the higher-trust path for `analyze()`
...
MCP CALL: consult(query, directory?, include?, exclude?, maxFiles?, detail?)
    │
    └── CURRENT: returns TEMPORARILY_PAUSED with alternatives.

PLANNED/PAUSED CONSULT DESIGN:
    │
...
MCP CALL: build_map(directory, embeddings?, exclude?)
    ...
    ├── ast-bridge.js:attachStructuralAnalysis() → imports, functions, classes, best-effort calls per file
    │   NOTE: analyze's higher-trust cross-file call graph still comes from ast-project.js/tree-sitter
...
MCP CALL: query_graph(symptom, directory?, maxResults?)
    ...
    ├── inspectGraphFreshness() → self-heal small stale KGs or flag needsFullRebuild
...
MCP CALL: doctor(directory?, checkFreshness?)
    │
    ├── Load .unravel/knowledge.json + .unravel/meta.json
    ├── inspectGraphFreshness() if enabled
    ├── doctorCodex() → stale/unreliable codex counts
    ├── describeEmbeddingProvider() → gemini/none/local readiness
    └── Return {status, projectRoot, kg, memory, freshness, embeddings, suggestions}
...
MCP CALL: analyze(files[], symptom, detail?)
    ...
    ├── pattern-store.js:matchPatterns() → top-5 structural patterns; agent-facing hints require confidence ≥0.65
...
The CLI also exposes the same diagnostics core used by MCP `doctor`:

```bash
node unravel-mcp/cli.js doctor <project> --format json
```

It reports KG presence, file/node/edge/call-edge counts, memory-layer counts, freshness, and embedding readiness. This is useful before demos because it proves what Unravel already knows before the agent starts reasoning.

---

## Reliability Harness and Benchmarks

As of 2026-05-01, Unravel MCP has a repeatable proof loop instead of only manual testing.

### Test layers

- **MCP contract tests:** start the real MCP server over stdio and verify the stable tool surface and response shapes.
- **Direct module tests:** register individual tool modules against a fake MCP server and mock dependencies to test important branches quickly.
- **Unit tests:** cover hashing, freshness, provider diagnostics, routing boosts, and stale embedding marking.

Current expected MCP tool list:

```text
analyze, build_map, consult, doctor, query_graph, query_visual, verify
```

`consult` must remain `TEMPORARILY_PAUSED` until its output quality gates are met.

### Benchmark modes

```bash
node validation/run-benchmark.js --mode routing
node validation/run-benchmark.js --bug super-bug-ghost-tenant --mode full
node validation/run-agent-eval.js --bug super-bug-ghost-tenant
node validation/run-demo.js super-bug-ghost-tenant
```

- **routing mode:** runs `build_map -> query_graph -> analyze` for every package with `ground-truth.md`. It checks whether the root-cause file appears in top-K and whether the analyze response contract is valid. It does not pretend RCA is solved.
- **full mode:** also runs deterministic diagnosis + `verify`, but only for bugs with a registered local diagnosis strategy. Unsupported bugs report `diagnosisSupported:false`, not fake failures.
- **agent eval mode:** runs a no-API A/B comparison today: scripted baseline diagnosis versus scripted Unravel-guided diagnosis. It saves both transcripts and scores to `agent-eval.json`. This proves the evaluation harness and score format; it is **not yet** a live LLM comparison.
- **clean outputs:** benchmark packages are copied to ignored temp workspaces before `build_map`, and result JSON goes to `validation/.tmp-results/` by default. Use `--out validation/results` only when intentionally refreshing committed historical outputs.

Current measured baseline from `node validation/run-benchmark.js --mode routing`:

```text
22 ground-truth packages
22/22 analyze contracts passed
19/22 root-cause files routed into top-K
0 generated benchmark .unravel fixture diffs
```

Current no-API A/B eval from `node validation/run-agent-eval.js --bug super-bug-ghost-tenant`:

```text
Engine: scripted (no API)
Baseline: 2/6
Unravel-guided: 6/6
Delta: +4/6
Saved transcript: validation/.tmp-results/super-bug-ghost-tenant/agent-eval.json
```

Important honesty note: this is a harness proof, not the final acquisition-grade claim. The next evaluation step is plugging in a real agent/model runner or manual blind transcripts using the same result shape.

### Manual Blind Trials

Manual blind trials use a human/agent evaluator as the LLM:

1. Read `symptom.md` and source files only.
2. Write a baseline diagnosis before opening `ground-truth.md`.
3. Run Unravel evidence (`analyze`, and when needed `build_map/query_graph`) and write the guided diagnosis.
4. Call `verify` on the guided diagnosis.
5. Only then open `ground-truth.md` and score honestly.

Current manual blind result:

```text
2026-05-01 | b-19-phantom-gate
Category: NO_BUG_FOUND / anti-sycophancy
Baseline diagnosis: no backend auth bypass in supplied files; likely SPA shell/client route behavior outside AdminRouter.
Unravel-guided diagnosis: AdminRouter.ts:22 gates admin API through requireAdmin before route dispatch; no provided backend bug.
verify: PASSED
ground truth: no bug in provided files; admin API routes correctly protected; SPA shell rendering without data is expected.
```

Additional manual blind checks:

```text
2026-05-01 | b-06-silent-await
Category: ASYNC_ORDERING
Diagnosis: DatabaseService.init() calls async connect() without await, and Application.bootstrap() calls db.init() without await.
Unravel signal: CLI detected floating promise at DatabaseService.ts; query_graph routed root-cause file into top-K.
verify: PASSED after exact citations were repaired.
grade reference: validation/results/b-06-silent-await/grade.md gives Unravel 6/6.

2026-05-01 | b-17-unstable-config
Category: React performance / unstable dependency
Diagnosis: ReportDashboard creates an inline config object every render; ReportPanel useMemo depends on config by reference, so runHeavyAggregation reruns.
Unravel signal: query_graph routed root-cause file into top-K; cross-file graph connected ReportPanel -> runHeavyAggregation.
verify: diagnosis matched ground truth, but manual verify attempt exposed citation-matcher friction on TSX evidence strings.
grade reference: validation/results/b-17-unstable-config/grade.md gives Unravel 6/6.

2026-05-01 | b-20-locked-key
Category: keyboard layout / layer boundary
Diagnosis: ShortcutHandler looks up shortcuts with layout-dependent event.key; robust fix is event.code / physical key binding, not a German translation table.
Unravel signal before structural-fact patch: analyze contract passed, but query_graph top-K routing missed ground-truth scoring for this layer-boundary case.
Unravel signal after structural-fact patch: CLI/analyze now surfaces ShortcutHandler.ts event property access map including .key@L34 and .code@L40. This is a lead, not proof; the keyboard-layout symptom/spec still supplies the semantic meaning.
verify: PASSED with exact source citations.
grade reference: validation/results/b-20-locked-key/grade.md gives Unravel 6/6 on the corrected full-file run.

2026-05-01 | b-15-bootstrap-deadlock
Category: null injection / constructor argument flow
Diagnosis: bootstrap constructs UserService with `null as unknown as AuthService`, then UserService later dereferences that dependency.
Unravel signal after structural-fact patch: CLI/analyze surfaces bootstrap.ts:6 and Gateway.ts:37 as `new UserService(arg0=null as unknown as AuthService)`. This closes the old gap where mutation chains saw the class identity but not the constructor argument value.
```

Why this matters: Unravel must not only find bugs. It must also refuse to invent bugs when the symptom is misleading and the code is correct.
