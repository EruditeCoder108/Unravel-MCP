...
import { getProjectDiagnostics } from './server/diagnostics.js';
...
function formatAnalysisForAgent(payload, detail = 'standard') {
    ...
    const structuralFactCount = (astRaw.constructorCallSites || []).length + (astRaw.propertyAccesses || []).length;
    ...
    } else if (detectorsFired === 0 && patternMatchCount === 0 && structuralFactCount > 0 && circleFindings.length === 0) {
        critLines.push('');
        critLines.push('NOTE: no high-confidence bug detector fired.');
        critLines.push('Unravel did produce structural facts (constructor args / property accesses).');
        critLines.push('Use them as leads for value-flow or semantic-property hypotheses, not as proof by themselves.');
    }
