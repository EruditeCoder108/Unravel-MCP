...
export function verifyClaims(result, codeFiles, astRaw, crossFileRaw, mode, symptom = '') {
    ...
    function fragmentAtClaimedLine(fileData, lineNum, fragment) {
        if (!fileData || !fragment) return false;
        const frag = fragment.trim();
        if (frag.length < 5) return false;
        const idx = lineNum - 1;
        if (idx < 0 || idx >= fileData.lines.length) return false;
        const lineCount = Math.max(1, frag.split(/\r?\n/).length);
        const window = fileData.lines.slice(idx, Math.min(fileData.lines.length, idx + lineCount)).join('\n');
        return window.replace(/\s+/g, ' ').includes(frag.replace(/\s+/g, ' '));
    }
    ...
    if (Array.isArray(evidenceList)) {
        ...
        for (const e of evidenceList) {
            ...
            const locPairs = extractFileLinePairs(e);
            if (fileRefs.length === 0 || locPairs.length === 0) {
                console.warn('[Verify] Check1 FAIL: evidence lacks file:line citation (+0.2 penalty)');
                failures.push({ claim: e, reason: 'evidence must include a file:line citation with literal source text' });
            ...
            const fragment = stripCitationPrefix(e);
            for (const { file, line } of locPairs) {
                const fileData = findFile(file);
                if (!fileData) continue;
                if (line > fileData.lines.length) {
                    console.warn(`[Verify] Check1 LINE FAIL: ${file} line ${line} > maxLine ${fileData.lines.length} (+0.2 penalty)`);
                    failures.push({ claim: e, reason: `evidence cites ${file} line ${line}, but file has only ${fileData.lines.length} lines` });
                    confidencePenalty += 0.2;
                    continue;
                }
                if (!fragmentAtClaimedLine(fileData, line, fragment)) {
                    console.warn(`[Verify] Check1 LINE FAIL: evidence literal not found at ${file} line ${line} (+0.2 penalty)`);
                    failures.push({ claim: e, reason: `evidence literal was not found at ${file} line ${line}` });
                    confidencePenalty += 0.2;
                }
            }
