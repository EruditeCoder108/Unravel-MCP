...
async function main() {
    ...
    if (format === 'sarif') {
        ...
        output = JSON.stringify({
            ...
            structuralFacts: {
                constructorCallSites: astRaw?.constructorCallSites || [],
                propertyAccesses: astRaw?.propertyAccesses || [],
            },
        ...
        const ctorSites = astRaw?.constructorCallSites || [];
        const propAccesses = astRaw?.propertyAccesses || [];
        const suspiciousCtorSites = ctorSites.filter(s => (
            s.flags?.hasNullArg || s.flags?.hasUndefinedArg || s.flags?.hasTypeAssertionArg
        ));
        ...
        if (suspiciousCtorSites.length) {
            lines.push(`CONSTRUCTOR ARGUMENT FACTS (${suspiciousCtorSites.length}):`);
            suspiciousCtorSites.slice(0, 8).forEach(s => {
                const args = (s.args || []).map(a => `arg${a.index}=${a.text || a.kind}`).join(', ') || 'no args';
                lines.push(`  [LEAD]  ${s.file}:${s.line} -> new ${s.className}(${args})`);
            });
        }
        ...
        for (const access of propAccesses) {
            ...
            if (!propGroups.has(key)) propGroups.set(key, { object: access.object, file: access.file, props: new Map() });
        ...
        const interestingPropGroups = [...propGroups.values()].filter(g => {
            const objectName = g.object.split('.').pop();
            return g.props.size >= 2 || ['event', 'evt', 'req', 'request', 'config', 'props', 'params'].includes(objectName);
        });
        if (interestingPropGroups.length) {
            lines.push(`PROPERTY ACCESS FACTS (${interestingPropGroups.length} groups):`);
            interestingPropGroups.slice(0, 8).forEach(g => {
                const props = [...g.props.entries()]
                    .slice(0, 8)
                    .map(([prop, hits]) => `.${prop}@L${hits[0].line}`)
                    .join(', ');
                lines.push(`  [LEAD]  ${g.file || '?'} -> ${g.object}: ${props}`);
            });
        }

        if (!races.length && !promises.length && !stale.length && !ctor.length) {
            if (suspiciousCtorSites.length || interestingPropGroups.length) {
                lines.push('No confirmed detector fired. Structural leads above still need symptom/spec verification.');
            } else {
                lines.push('No confirmed structural detector fired in standard mode.');
            }
        }
